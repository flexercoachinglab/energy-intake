...
const EX_TYPE = [
  { id: "none",  label: "運動習慣なし", perFreq: 0 },
...
export default function App() {
  ...
  const [exType, setExType] = useState("none");
  const [freq, setFreq] = useState("f12");
  ...
  const r = useMemo(() => {
    ...
    const t = EX_TYPE.find((e) => e.id === exType);
    const fn = FREQ.find((f) => f.id === freq)?.n || 0;
    const eAdd = exType === "none" ? 0 : Math.min(t.perFreq * fn, 0.18);
  ...
  }, [sex, age, height, weight, job, fatigue, stepsMode, steps, lifestyle, exType, freq, lossKg]);
  ...
  const S = {
    ...
    lead: { fontSize: 13, color: C.sub, lineHeight: 1.7, margin: "0 0 20px" },
  ...
  return (
    <div style={S.wrap}>
      <div style={S.inner}>
        ...
        <h1 style={S.h1}>摂取カロリー設定</h1>
        <p style={S.lead}>目標に合わせた1日の摂取カロリーを、レンジ（幅）で提示します。</p>
        ...
        <div style={S.card}>
          <span style={S.label}>運動習慣（意図的な運動）</span>
          <div style={{ fontSize: 11, color: C.sub, marginBottom: 8 }}>種目</div>
          {EX_TYPE.map((e) => (
            <div key={e.id} style={S.opt(exType === e.id)} onClick={() => setExType(e.id)}>
              <div style={{ fontSize: 13, fontWeight: 600 }}>{e.label}</div>
            </div>
          ))}
          {exType !== "none" && (
            <>
              <div style={{ fontSize: 11, color: C.sub, margin: "10px 0 6px" }}>頻度</div>
              <div style={S.segWrap}>
                {FREQ.map((f) => (
                  <div key={f.id} style={S.seg(freq === f.id)} onClick={() => setFreq(f.id)}>{f.label}</div>
                ))}
              </div>
            </>
          )}
        ...
        <div style={S.resultCard}>
          ...
          <div style={{ borderTop: `1px solid ${C.accent}33`, paddingTop: 16, marginTop: 16 }}>
            <div style={S.rangeLab}>減量推奨摂取レンジ（月−{lossKg.toFixed(1)}kg）</div>
            <div style={S.rangeVal}>
              <span style={S.mid(r.belowBMR ? C.danger : C.accentDeep)}>{r.intakeMid.toLocaleString()}</span>
              <span style={{ fontSize: 15, color: r.belowBMR ? C.danger : C.accentDeep, fontWeight: 700 }}>kcal/日</span>
            </div>
            <div style={S.band}>幅: {r.intakeLo.toLocaleString()} 〜 {r.intakeHi.toLocaleString()} kcal</div>
          </div>
          ...
          {r.belowBMR && (
            <div style={S.warnBox(C.danger)}>
              ...
              <div style={{ fontSize: 13, lineHeight: 1.8, color: C.ink }}>
                この摂取量（{r.intakeMid.toLocaleString()}kcal）は基礎代謝（{r.bmr.toLocaleString()}kcal）を割り込みます。
